...
class IncidentHandler:
    def __init__(self):
        self.incidents = pd.DataFrame(
            columns=["Id", "Line", "Locations", "Cause", "StartDate", "EndDate"]
        )
    ...
    def updateIncident(self, id, time, text):
        newLocations, newCause, newEndDate = editEntryPipeline(
            text,
            time,
            self.incidents.loc[id]["Locations"],
            self.incidents.loc[id]["Cause"],
            self.incidents.loc[id]["EndDate"],
        )
        change = False
