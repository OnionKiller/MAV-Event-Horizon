...
from .models import MAVEvent, MAVEventDump
...
class FeedParser(object):
        ...
        try:
            event = MAVEvent(
                ...
                last_modification=datetime.fromtimestamp(
                    mktime(feedEntry["published_parsed"])
                ),
            )
        except ValueError as e:
            logging.error(
                f"Feed is missing a field: {str(e)}. \n feedEntry : {str(feedEntry)}"
            )
    ...
    def dump_entry(feedEntry, uuid) -> MAVEventDump:
        return MAVEventDump(event_dict=feedEntry, event_uuid=uuid)
